...
class DispController extends Controller
{
    ...
    public function updateList() {
        if(Input::post('start_date') and Input::post('finish_date')) {
            \Log::info('asdasd');
            $data = Disp::where('start_date','>=',Input::post('start_date'))->where('finish_date','<=',Input::post('finish_date'))->get();
            return view('Disp')->with('data',$data);
        } elseif(Input::post('start_date') and !Input::post('finish_date')) {
            $data = Disp::where('start_date','>=',Input::post('start_date'))->where('finish_date','<=','2100/12/12')->get();
            return view('Disp')->with('data',$data);
        } elseif(!Input::post('start_date') and Input::post('finish_date')) {
            $data = Disp::where('start_date','>=','1900/12/12')->where('finish_date','<=',Input::post('finish_date'))->get();
            return view('Disp')->with('data',$data);
        } else {
            $data = Disp::getAllData();
            return view('Disp')->with('data',$data);
    ...
    public function showCertificate($name) {
    	...
    	foreach ($array_data as $datas) {
            ...
            $temp_detail = DispDetail::where('name', $name)->where('peserta', $datas)->first();
