...
class Disp extends Model
{
	...
	public static function getSum() {
		return DB::table('disp')->count();
	}

	public static function jumlahLulus($year) {
		$datas = DB::table('disp')->whereYear('start_date', '<=', $year)
		...
		foreach($datas as $data) {
			$sum = $sum + DB::table('disp_detail')->where('name', $data->name)->where('participant_status', 'Certified')->count();
		}
		return $sum;
	}
	...
	public static function getByMonthProgram($year, $program, $month) {
		return DB::table('disp')->whereYear('start_date', $year)
								->where('telkom_main', $program)
